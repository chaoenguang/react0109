...
const { createArticle, listArticle, saveArtivle, deleteArticle} = require('./db_utils');
...
app.post('/article/delete', async (req, res) => {
    const {_id, title, content, author } = req.body.record;
    await deleteArticle({_id, title, content, author })
    res.send({
        code:100,
        data:''
    })
